...
export const deleteFavoritesByUserIdService = async (userId) => {
  const result = await pool.query(
...
export const deleteFavoritesByStationIdService = async (station_id) => {
  const result = await pool.query(
    `DELETE FROM favorites WHERE station_id = $1 RETURNING *`,
    [station_id]
  );
  return result.rows; 
};
